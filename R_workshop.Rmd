...
Earlier we used a dataset that was saved in a text file with columns delimited by tabulations (using the `read.table()` function). However, many of you save your data in excel files. We can install a package to get a new function specialised in reading excel files by running `install.packages('readxl')`. Once you've done this, add the following line at the top of your script:
...
Let's load a new dataset to work with (found on the [Workshop repository](https://github.com/isabelle-laforest/RworkshopUdS_2025/tree/main/data) )
