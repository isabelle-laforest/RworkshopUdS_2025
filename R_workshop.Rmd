...
### Partial solution (we'll finish tomorrow)
...
In this plot, our H0 is that there is no difference in the mean of the Y variable between the two groups.
...
## Comparing the mean across more than two groups
...
#### Check the conditions to apply ANOVA 
...
### Check Normality of Moss using the function


`shapiro.test()`
Get Moss as close to normality as possible (reciprocal, logarithm, cube root, square root, and square)
...
pacman::p_load(tidyverse,patchwork,lme4,lmerTest,emmeans)
```
### Load roo data

```{r , message=FALSE, warning=FALSE}
roo <- read_xlsx('data/Marco_kangoroos.xlsx')
```
...
```{r lm_model1}
model_lm1 <- lm(Mass ~ Age, data = roo_filtered)
summary(model_lm1)
```
With only Sex:

```{r lm_model2}
model_lm2 <- lm(Mass ~ Sex, data = roo_filtered)
summary(model_lm2)
```

With Mass and Sex but no interaction:

```{r lm_model3}
model_lm3 <- lm(Mass ~ Age + Sex, data = roo_filtered)
summary(model_lm3)
```
...
```{r lm_model4}
model_lm4 <- lm(Mass ~ Age * Sex, data = roo_filtered)
summary(model_lm4)
```
