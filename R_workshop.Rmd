...
```{r}
ggplot(tomato_mdf, aes(x = chlorotic.area)) + 
  geom_histogram()
```
...
```{r}
ggplot(tomato_mdf, aes(x = total.area, y = chlorotic.area)) + 
...
```{r}
ggplot(tomato_mdf, aes(x = Treatment, y = total.area)) +
...
tomato_mdf %>% 
  ...
  labs(title = "Boxplot", x = "Treatment", y = "Proportion of chlorotic area") 
...
1. Make a violin plot of the proportion of chlorosis for each treatment state using `geom_violin()`
2. Add the points on top of the violins
3. Add labels 
3. Map the shape of the points to the `rep` variable (hint: you can reuse the `aes()` function inside any geom, here's a [reference](https://www.sthda.com/english/wiki/ggplot2-point-shapes))
4. Facet this plot by infection state
5. Use the minimal theme
